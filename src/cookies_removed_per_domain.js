...
chrome.storage.onChanged.addListener(function(changes,areaName){
    ...
    if (changes.browserData != null){

        var data = changes.browserData;
        ...
        for (i = 0; i < Object.keys(data).length; i++) {
            for(j = 0; j < data[Object.keys(data)[i]].count; j++){
                var host = getLocation(data[Object.keys(data)[i]].entries[j].path).hostname
                if (host in pathDict) {
                    pathDict[host] += data[Object.keys(data)[i]].entries[j].size
                }
